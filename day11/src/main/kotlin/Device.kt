...
class Device(private val serialNumber: Int) {
    ...
    private val summedAreaTable: Map<Cell, Int> = run {
        ...
        allPoints.forEach { cell ->
            ...
            val leftSum  = result.getOrDefault(cell.shiftBy(dx = -1), 0)
            val aboveSum = result.getOrDefault(cell.shiftBy(dy = -1), 0)
            val dupeSum  = result.getOrDefault(cell.shiftBy(dx = -1, dy = -1), 0)
            //@formatter:on
